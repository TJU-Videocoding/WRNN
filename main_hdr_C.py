...
writer = SummaryWriter(log_dir='tensorboard/TIP/HDR_YUV_C')
...
for epoch in range(300):
    ...
    for i, data in enumerate(iterator):
        ...
        pred_C = net.forward(img.float().permute(0, 3, 1, 2))
        loss = loss_l1(pred_C.float(), C.unsqueeze(1).float())
    ...
    print('epoch:', epoch, ' label:', C.unsqueeze(1).float(), ' pred:', pred_C.float())

    with open('logs/TIP/log_HDR_YUV_C.txt', 'a') as f:
    ...
    if (epoch+1) % 1 == 0:
        ...
        for i, (img, para) in enumerate(test_data_loader):
            with torch.no_grad():
                ...
                print('index:', i, ' label:', C.unsqueeze(1).float(), ' pred:', pred_C.float())
                diff = abs(pred_C.float() - C.unsqueeze(1).float())
        ...
        print('Saving Model.....')
        torch.save(net, "./TIP/model_HDR_YUV_C_"+str(epoch)+".pkl")
