...
def parse_hdr(excel_root, data_root):
    image_info = []
    excel = openpyxl.load_workbook(os.path.join(excel_root))
    sheet = excel['data']
    for row in sheet.values:
        seq_name, poc, ctu_addr, C, K = row
        if C is None:
            continue
        para = (C, K)
...
class HDRDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.image_info = parse_hdr(self.excel_root, self.data_root)
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            img = img / 1023.0
            img_Y = numpy.expand_dims(img[:, :, 0], 2)
            outputs = [
                img,
                para
            ]
...
class HDRDataset_Y(torch.utils.data.Dataset):
    def __init__(
            self,
            excel_root,
            data_root
...
def parse_hdr_seq(data_root):
    image_info = []
    for filename in os.listdir(data_root):
        file = os.path.join(data_root, filename)
...
class HDRTestSeqs(torch.utils.data.Dataset):
    def __init__(
            self,
            data_root
    ):
        self.data_root = data_root
        self.image_info = parse_hdr_seq(self.data_root)

    def __getitem__(self, index):
        image_dir = self.image_info[index]
        try:
            img = readSeq(image_dir)
            img = img / 1023.0
            outputs = [
                image_dir,
                img
...
#
# train_data = HDRDataset('/data0/Yuan/HDR_data/20221117/label/train_label.xlsx',
#                         '/data0/Yuan/HDR_data/20221117/Train/train_set/')
...
class HDRTestDataset_Y(torch.utils.data.Dataset):
    def __init__(
            self,
            data_root
    ):
